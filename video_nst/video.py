"""
    ...
    Processing consists out of 5 stages:
        ...
        3. Stylize dumped frames using external NST repo (pytorch_nst_feedforward) integrated as a git submodule
...
import shutil
import sys
# Enables this project to see packages from pytorch_nst_feedforward submodule (e.g. utils)
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'pytorch_nst_feedforward'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--specific_videos", type=str, help="Process only specific videos in data/", default=['rahul.mp4'])
    ...
    parser.add_argument("--model_name", type=str, help="Model binary to use for stylization", default='mosaic.pth')
    ...
    nst_submodule_path = os.path.join(os.path.dirname(__file__), 'pytorch_nst_feedforward')
    assert os.path.exists(nst_submodule_path), 'Please pull the pytorch_nst_feedforward submodule to use this project.'
